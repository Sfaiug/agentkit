...
still needs attention (see below), or an `exhausted` run the tick cannot resume (rounds spent, a stopped `git`, no verdict), handed back or not, until `ak run resume <id>` or an acknowledgement, is `needs you` (`run <id> parked: <reason>`); a harness turn in flight is `working`;
