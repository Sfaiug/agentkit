...
parked `stalled`, or `exhausted` on a window or a dead reviewer, which the tick resumes by itself unless a later merged run replaced it (then it is settled and reads `done`); a seat that ended its turn on `ak wait <session>` is `working` with `waiting on <session>` while that session itself reads `working` by its own runs or turn, never by a wait of its own, until the tick has told the seat that session stopped (below) or the seat's next `ak wait` or `ak notify`; a harness turn in flight is `working`, parked run or not; an error with no automatic resume that
...
- `[providers.<name>]`: `mode` (`subscription` or `payg`). MiMo's comes from the global OpenCode `opencode.json`: a
  ...
  `accounts = ["default", "second"]` lists several subscriptions; `default` is the usual login, and Claude keeps another's
  worker token in `secrets/claude_oauth_token.second`, its login in `~/.claude-second` (`CLAUDE_CONFIG_DIR=~/.claude-second
  claude`). A worker turn runs on the account with most room; one refused or spent is parked alone, and the turn goes to
  the next before any other model. `ak usage` shows each account on its own row.
